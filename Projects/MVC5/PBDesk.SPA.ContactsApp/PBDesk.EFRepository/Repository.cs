...
namespace PBDesk.EFRepository
{
    public class Repository<T> : IRepository where T : IEntity
    {
        ...
        public List<T> InsertAll<T>(List<T> objs) where T : Entity
        {
            ...
            if (objs != null && objs.Count() > 0)
            {
                foreach(var obj in objs)
                {
                    try
                    {
                        result.Add(Insert<T>(obj));
                    }
                    catch(Exception ex)
                    {
                        throw new EFRepositoryException("Error while inserting data to database via DbContext.", "Repository.InsertAll(objs)", ex);
                    }
                }
        ...
        public int UpdateAll<T>(List<T> objs, string primaryKeyName = "Id") where T : Entity
        {
            int result = 0;
             if(objs != null && objs.Count() > 0 )
             {
